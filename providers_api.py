...
class MaterialSymbolsProvider(IconProvider):
    ...
    def search(self, query: str, page: int = 1, per_page: int = 20) -> SearchResult:
        """Search Material Symbols using GitHub API"""
        try:
            # GitHub API endpoint for searching code in the repository
            # We search for SVG files containing the query
            search_query = f"{query} in:path extension:svg repo:{self.repo}"
            encoded_query = quote(search_query)

            api_url = f"https://api.github.com/search/code?q={encoded_query}&per_page={per_page}&page={page}"
            ...
            for item in data.get('items', []):
                ...
                if path.endswith('.svg'):
                    ...
                    icon_name = os.path.basename(path).replace('.svg', '').replace('_', '-')

                    # Build URLs
                    raw_url = f"https://raw.githubusercontent.com/{self.repo}/master/{path}"
            ...
            if e.code == 403:
                # Rate limit hit, return empty
                print(f"GitHub API rate limit hit: {e}")
            else:
